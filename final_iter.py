...
df_reliability,mean_bias,sd_bias=3,0,15
iter_num=20
    
CI_global=np.zeros(iter_num)
CI_incentivized=np.zeros(iter_num)
for s in range(iter_num):

    ########################## Step 1, assignment ############################
    
    ##first define the assign function to generate peer-review assignments
    ##the paramters are n (total # of PIs) and m(# of proposals each PI review)
    def assign(n=10,m=7):
        """
    ...
    #creat function to simulate true scores using truncated normal distribution
    ...
    def prop_scores(n=50, df_reliability=6,mean_bias=0,sd_bias=5):
        ...
        for i in range(0,n):
            for j in range(0,n):
                ...
                prop_scores[i,j]=np.random.normal(true_scores[j]+bias[i],
                                                  reliability[i])
...
#    plt.show()
    ...
    CI_global[s]=compCI(prop_global_ranks, prop_true_ranks, n)
#    print('CI_global',CI_global)
    ...
    quality_index_diff= abs(simulated_assignment_ranks - 
                            global_assignment_ranks_ranks)
    QI=quality_index_diff.sum(axis=1)
    
    #print('pi_quality_index',pi_quality_index)
    
    ##compute the average difference in score between ranked proposals
    ...
    CI_incentivized[s]=compCI(prop_incentivized_ranks, prop_true_ranks, n)
...
CI_global_avg=np.mean(CI_global)
CI_incentivized_avg=np.mean(CI_incentivized)
print('CI_global_avg',CI_global_avg)  
print('CI_incentivized_avg',CI_incentivized_avg)       
